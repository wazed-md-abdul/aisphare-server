...
export async function generateContent(req, res) {
    ...
    try {
        openSSE(res);
        const full = await openRouterService.generateContent({ subject, topic, difficulty, outputType, length }, (t) => sendToken(res, t));
...
export async function chat(req, res) {
    ...
    try {
        openSSE(res);
        const full = await openRouterService.chat({ studentName: context?.studentName ?? "student", college: "EduSphere AI", ...context }, messages, (t) => sendToken(res, t));
        const turn = [
            ...messages,
...
export async function updateDocumentTags(req, res) {
    const { tags } = req.body;
    const doc = await DocumentModel.findOneAndUpdate({ _id: req.params.id, owner: req.userId }, { $set: { tags } }, { new: true });
    if (!doc) {
...
export async function recommendHistory(req, res) {
